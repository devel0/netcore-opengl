        ...
        get
        {
            ...
            if (_BuiltinFontPathfilename is null)
            {
                ...
                if (!File.Exists(fontPathfilename))
                {
                    ...
                    var buf = GetEmbeddedResourceNames(GLContext.netcore_opengl_core_assembly)
                        .First(w => w.RegexMatch(resourceName) > 0)
                        .Fn(name => GetEmbeddedFileContentBytes(GLContext.netcore_opengl_core_assembly, name));

                    if (buf is null) throw new Exception($"can't read embedded resource [{resourceName}] from assembly [{GLContext.netcore_opengl_core_assembly.FullName}]");
